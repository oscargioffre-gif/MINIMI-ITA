...
# v3: ISIN e nomi da mappa statica (no più .info → no rate-limit)
...
from universe import get_metadata
...
def analyze_ticker(
    ...
    Nome e ISIN da mappa statica (zero chiamate API).
    """
    # Metadata sempre dalla mappa statica - immediato, no rate-limit
    nome, isin = get_metadata(ticker)
    result = TickerResult(
        ticker=ticker,
        nome=nome,
        isin=isin,
        anni_lookback=years_lookback,
    )
...
def errors_to_dataframe(errors: list[TickerResult]) -> pd.DataFrame:
    """DataFrame degli errori per la sezione Debug, con nome/ISIN se disponibili."""
    if not errors:
        return pd.DataFrame()
    return pd.DataFrame([
        {
